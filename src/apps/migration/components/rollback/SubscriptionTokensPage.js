...
const SubscriptionTokensPage = ({
	...
	const handleFileChange = (event) => {
		...
		if (selectedFile) {
			...
			// API call to validate the CSV file for rollback
			apiFetch({
				path: window.wcpsm_migration_data.endpoints
					?.validate_subscription_tokens_rollback,
				method: 'POST',
				headers: {
	...
	return (
		<div className="wpsm-migration-page">
			<h2>Import Subscription Tokens for Rollback</h2>
			<p className="text-muted">
				Please upload a CSV file containing your subscription tokens for
				rollback. Ensure the file format is correct.
			</p>
			<FormFileUpload
				variant="secondary"
				type="file"
			...
			<div>
				...
				{errorMessage && <p className="text-failure">{errorMessage}</p>}
			</div>
