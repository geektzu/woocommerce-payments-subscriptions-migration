...
class WCPSM_Rest_Subscription extends WP_REST_Controller {
	 ...
	 * Get the single instance of the class.
	 *
	 * @return WC_Payments_Customer_Service Returns customer id according to the mode.
	 */
	...
	/**
	 * Process the migration of a subscription.
	 *
	 * @param array  $subscription_data  The subscription data.
	 * @param string $origin_pm          The origin payment method.
	 * @param string $destination_pm     The destination payment method.
	 *
	 * @return bool
	 */
	...
	/**
	 * Migrate the selected subscriptions.
	 *
	...
	/**
	 * Dry run the migration of the selected subscriptions.
	 *
	 * @param WP_Rest_Request $request The request object.
	 * @return WP_Rest_Response The response object.
	 */
	...
	/**
	 * Get the source key for the payment method.
	 *
	...
	/**
	 * Get the customer id key for the payment method.
	 *
	 * @param string $method The payment method.
	 * @return string
	 */
	 ...
	 *
	 * @param array  $subscriptions_raw The raw subscription data.
	 * @param string $origin_pm         The origin payment method.
	 * @param string $destination_pm    The destination payment method.
	 * @return array
	 */
					...
					if ( $subscription ) {
						...
						if ( $source && $customer_id ) {
							...
							if ( isset( $file_data[ $search_key ] ) ) {
								...
								$existing_subscriptions = wcs_get_subscriptions(
									array(
	...
	/**
	 * Get the file contents of the CSV file that is uploaded for validation and preview of the first 5 rows.
	 *
	 * @param array $uploaded_file The uploaded file object.
	 * @return array
	 */
	private function get_file_contents( $uploaded_file ) {
		...
		$file_content = $filesystem->get_contents( $uploaded_file['tmp_name'] );
	...
	/**
	 * Validate the subscription tokens in the uploaded CSV file.
	 *
	 * @param WP_Rest_Request $request The request object.
	 * @return WP_Rest_Response The response object.
	 */
	public function validate_subscription_tokens( $request ) {
		...
		if ( empty( $files['file'] ) || UPLOAD_ERR_OK !== $files['file']['error'] || ! $user_id ) {
	...
	/**
	 * Check if the custom orders (HPOS) table is enabled.
	 *
	 * @return bool
	 */
	private function is_hpos() {
		if ( class_exists( 'Automattic\WooCommerce\Utilities\OrderUtil' ) && Automattic\WooCommerce\Utilities\OrderUtil::custom_orders_table_usage_is_enabled() && 'yes' !== get_option( 'woocommerce_custom_orders_table_data_sync_enabled' ) ) {
			return true;
		}
		return false;
	}
	...
	/**
	 * Get the subscriptions by payment method.
	 *
	 * @param string $method The payment method.
	 * @return array
	 */
	...
	/**
	 * Delete the subscription migration file.
	 *
